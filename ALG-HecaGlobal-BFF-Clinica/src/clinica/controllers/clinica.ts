...
import { estadosCuotas } from '../models/estadosCuotas';
import axios, { AxiosResponse } from 'axios'
import { usuario, usuarioToken } from '../models/usuario'
...
class Controller {
    ...
    listarestadocuota = async (req: Request, res: Response, next: NextFunction) => {
        try {
            let result: AxiosResponse = await axios.get(urlServicioClinica + String(process.env.ESTADOSCUOTAS));
            let estadosCuotas: [estadosCuotas] = result.data;
    ...
    listarestadofinanciamiento = async (req: Request, res: Response, next: NextFunction) => {
        try {

            let result: AxiosResponse = await axios.get(urlServicioClinica + String(process.env.ESTADOSFINANCIAMIENTOS));
            let objlista = result.data.data;
            let listaEstados = objlista.map(function (data: any) {
                const estados = {
    ...
    listarestadofinanciamientosuperior = async (req: Request, res: Response, next: NextFunction) => {
        try {
            let result: AxiosResponse = await axios.get(urlServicioClinica + String(process.env.LISTARESTADOSFINANCIAMIENTOSUPERIOR));
            let objlista = result.data.data;
            let listaEstados = objlista.map(function (data: any) {
                const estados = {
                    idEstadoFinanciamiento: data.idestadofinanciamiento,
    ...
    listarproductogarantia = async (req: Request, res: Response, next: NextFunction) => {
        try {

            let result: AxiosResponse = await axios.get(urlServicioClinica + String(process.env.PRODUCTOGARANTIA));
            let objlista = result.data.data;

            let productosGarantias = objlista.map(function (data: any) {
                const estados = {
    ...
    listarenproceso = async (req: Request, res: Response, next: NextFunction) => {
        let idclinica: string = req.body.idclinica;

        try {
            let result: AxiosResponse = await axios.post(urlServicioClinica + String(process.env.LISTAFINANCIAMIENTOS), {
                idclinica: idclinica
            });

            if (result.data.status == 200) {
                let objlista = result.data.data;
                let enProceso = objlista.map(function (data: any) {
                    const proceso = {
                    ...
                    return proceso;
                }).filter((n: { idestadofinanciamie: number; }) => n.idestadofinanciamie < 600);
    ...
    listarconfinanciamiento = async (req: Request, res: Response, next: NextFunction) => {
        let idclinica: string = req.body.idclinica;
        ...
        try {
            ...
            if (result.data.status == 200) {
                ...
                let enProceso = objlista.map(function (data: any) {
                    ...
                    return proceso;
                }).filter((n: { idestadofinanciamie: number; }) => n.idestadofinanciamie === 700);
                res.status(200).json({ status: result.data.status, message: result.data.message, data: enProceso })
            }
            else {
                res.status(200).json({ status: result.data.status, message: result.data.message, data: result.data.data })
    ...
    listarfinanciamientosproceso = async (req: Request, res: Response, next: NextFunction) => {
        let idclinica: string = req.body.idclinica;
    ...
    obtenerdatostratamiento = async (req: Request, res: Response, next: NextFunction) => {

        let idclinica: string = req.body.idclinica;
        let idusuario: string = req.body.idusuario;
        let idpaciente: string = req.body.idpaciente;
        let idtratamiento: string = req.body.idtratamiento;

        try {
            let result: AxiosResponse = await axios.post(urlServicioClinica + String(process.env.OBTENERDATOSTRATAMIENTO), {
                idclinica: idclinica,
                idusuario: idusuario,
            ...
            if (result.data.status == 200) {
                ...
                let datosTratamiento = objResultado.map(function (data: any) {
                    const tratamiento = {
    ...
    crearfinanciamiento = async (req: Request, res: Response, next: NextFunction) => {

        let idclinica: number = req.body.idclinica;
        let rutPaciente: string = req.body.rutPaciente;
        let numeroPaciente: number = req.body.numeroPaciente;
        let nombrePaciente: string = req.body.nombrePaciente;
        let apellidoPaciente: string = req.body.apellidoPaciente;
        let emailPaciente: string = req.body.emailPaciente;
        let rutPagador: string = req.body.rutPagador;
        let emailPagador: string = req.body.emailPagador;
        let idProductoGarantia: number = req.body.idProductoGarantia;
        //let numeroTratamiento: number = req.body.numeroTratamiento;
        let idDoctor: string = req.body.idDoctor;
        let duracionTratamiento: number = req.body.duracionTratamiento;
        let montoTratatamiento: number = req.body.montoTratatamiento;
        let montoPagoInicial: number = req.body.montoPagoInicial;
        let financiamientoASolicitar: number = req.body.financiamientoASolicitar;
        let nombreOrtodoncista: number = req.body.nombreOrtodoncista;
        let telefonoPaciente: number = req.body.telefonoPaciente;

        try {
            let result: AxiosResponse = await axios.post(urlServicioClinica + String(urlCreaFinanciamiento), {
                idclinica: idclinica,
            ...
            if (result.data.status == 200) {
                ...
                let token = await generaToken(usuarioToken, process.env.JWT_TOKEN_EXPIRES_CREAR_CUENTAS);
                ...
                await this.enviarcorreoregistro(req, url);
                res.status(200).json({ status: result.data.status, message: result.data.message, data: result.data.data })
            }
            else {
                res.status(200).json({ status: result.data.status, message: result.data.message, data: result.data.data })
    ...
    actualizarnotainterna = async (req: Request, res: Response, next: NextFunction) => {
        let idtratamiento: string = req.body.idtratamiento;
        let textonotainterna: string = req.body.textonotainterna;
        try {
            let result: AxiosResponse = await axios.put(urlServicioClinica + String(process.env.ACTUALIZARNOTAINTERNA), {
                idtratamiento: idtratamiento,
    ...
    obtenerrutaarchivo = async (req: Request, res: Response, next: NextFunction) => {

        let rutpagador: string = req.body.rutpagador;
        let idfinanciamiento: string = req.body.idfinanciamiento;
        let tipoarchivo: string = req.body.tipoarchivo;

        try {
            let result: AxiosResponse = await axios.post(urlServicioClinica + String(process.env.OBTENERRUTAARCHIVO), {
                rutpagador: rutpagador,
    ...
    getUsuarios = async (req: Request, res: Response, next: NextFunction) => {
        try {
            let result: AxiosResponse = await axios.get(urlServicioUsuario + String(process.env.LISTARUSUARIOS));
    ...
    validaringresousuario = async (req: Request, res: Response, next: NextFunction) => {

        let correo: string = req.body.correo;
        let clave: string = req.body.clave;
        try {
            let result: AxiosResponse = await axios.post(urlServicioUsuario + String(process.env.VALIDARINGRESOUSUARIO), {
                correo: correo,
                clave: clave
            });
            ...
            if(result.data.status === 200)
            {
                ...
                let token = await generaToken(usuarioToken);
    ...
    obtenerclaveusuario = async (req: Request, res: Response, next: NextFunction) => {
        let correo: string = req.body.correo;
        let nombre: string = req.body.nombre;
        try {
            let result: AxiosResponse = await axios.post(urlServicioUsuario + String(process.env.OBTENERCLAVEUSUARIO), {
                correo: correo,
                nombre: nombre
            });
            ...
            if (result.data.status == 200) {
                ...
                await this.enviarcorreorecuperarclave(req, result.data.data.clave);
    ...
    obtenerarchivo = async (req: Request, res: Response, next: NextFunction) => {
        let nombrearchivo: string = req.body.nombrearchivo;
        let rutpagador: string = req.body.rutpagador;
        let idtratamiento: string = req.body.idtratamiento;
        let tipoarchivo: string = req.body.tipoarchivo;
        if (!nombrearchivo) return res.status(200).json({ status: 400, message: 'Ingrese nombre archivo' });
        if (!rutpagador) return res.status(200).json({ status: 400, message: 'Ingrese rutpagador' });
        if (!idtratamiento) return res.status(200).json({ status: 400, message: 'Ingrese id tratamiento' });
        if (!tipoarchivo) return res.status(200).json({ status: 400, message: 'Ingrese tipo archivo' });
        try {
            let result: AxiosResponse = await axios.put(urlServicioClinica + String(process.env.ACTUALIZARIMPRESOARCHIVO), {
                rutpagador: rutpagador,
                idtratamiento: idtratamiento,
                tipoarchivo: tipoarchivo
            });
            if (result.data.status == 200) {
                ...
                let archivobase64: AxiosResponse = await axios.post(urlServicioObtenerArchivo, {
                    fileName: lastItem,
    ...
    enviarcorreoregistro = async (req: Request, urlCrear: string,) => {
        let templateId: string = req.body.pacientePagador === 1 ? String(pagadorIgualPaciente) : String(pagadorDiferentePaciente);
        let to: string = req.body.emailPagador;
        let nombre: string = req.body.nombrePaciente + ' ' + req.body.apellidoPaciente;
        let pagador: string = req.body.nombrePaciente + ' ' + req.body.apellidoPaciente;
        let paciente: string = req.body.nombrePaciente === "" ? " " : req.body.nombrePaciente;
        let codigo: string = '';
        let url: string = urlCrear;
        let asunto: string = 'Crear cuenta';
    ...
    enviarcorreorecuperarclave = async (req: Request, clave: string) => {
        let templateId: string = String(recuperarContrasenaCorreo);
        let to: string = req.body.correo;
        let nombre: string = req.body.nombre;
        let codigo: string = clave;
    ...
    generarPagare = async (req: Request, res: Response, next: NextFunction) => {
        let rutpagador: string = req.body.rutpagador;
        let idtratamiento: string = req.body.idtratamiento;
        if (!rutpagador) return res.status(200).json({ status: 400, message: 'Ingrese rut pagador' });
        if (!idtratamiento) return res.status(200).json({ status: 400, message: 'Ingrese id tratamiento' });
        
        let resultObtenerArchivo: AxiosResponse = await axios.post(urlServicioClinica + String(process.env.OBTENERRUTAARCHIVO), {
            rutpagador: rutpagador,
            idtratamiento: idtratamiento,
            tipoarchivo: 3
        });
        ...
        if(resultObtenerArchivo.data.status == 200)
        {
            ...
            await axios.post(urlServicioEliminarArchivo, {
                fileName: lastItem,
                carpetaBucket: carpetabucket
            });
        }

        let resultEliminacion: AxiosResponse = await axios.post(urlServicioClinica + String(process.env.ELIMINARARCHIVO), {
            rutpagador: rutpagador,
            idtratamiento: idtratamiento,
            idtipoarchivo: 3
        });
        ...
        let result: AxiosResponse = await axios.post(urlServicioClinica + String(process.env.OBTENERDATOSCONTRATOPAGARE), {
            rutpagador: rutpagador,
            idtratamiento: idtratamiento
        });
        if (result.data.status == 200) {
            ...
            let options = { 
                ...
                footerTemplate:
                  '<style>span{display: block; height: 4mm; text-align: right; font-size:11px; color:#000; font-family: Arial, sans-serif !important; width:98% !important;}</style><span>Página <label class="pageNumber"></label>/<label class="totalPages"></label></span>',
                margin: {
                  top: "0.5cm",
                  bottom: "1cm",
                  right: "0.5cm",
            ...
            pdf.generatePdf(file, options, async function (err: any, pdfBuffer: any) {
                try {
                    if (err) {
                        return res.json({ err });
                    } else {
                        ...
                        let ruta: AxiosResponse = await axios.post(urlServicioArchivo, {
                            fileNameIn: nombreFileName,
                            fileSource: 'data:application/pdf;base64,' + Buffer.from(pdfBuffer).toString('base64')
                        });
                        let resultArchivo: AxiosResponse = await axios.post(urlServicioClinica + String(process.env.GUARDARARCHIVO), {
                            ruta: ruta.data,
                            idtratamiento: idtratamiento,
                            rutpagador: rutpagador,
                            idtipoarchivo: 3
